...
export function HomeFeed() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await feedApi.get({ limit: 20 });
      setPosts(normalizeArray(data));
    } catch (err) {
      if (process.env.NODE_ENV === 'development') console.error('[home feed]', err);
      setPosts([]);
        ...
        <div className="space-y-4">
          {safePosts.map((post, idx) => {
            if (isCommonSessionPost(post)) {
              ...
              if (ctx) {
                return (
                  <div key={post?.id || `feed-common-${idx}`} className="space-y-2">
                    {post.feed_source ? (
